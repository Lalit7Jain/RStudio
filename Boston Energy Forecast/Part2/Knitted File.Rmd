...
setwd("C:\\Users\\Lipsa\\Desktop\\ADS\\Assignment 2")
...
After looking in to the information pulled by the WeatherDatapacla is pulling information for each hour interval. We can get confirmed with the following function.

```{r,message=FALSE,warning=FALSE}
table(WeatherData$date)
```

After looking at the tabular values, we deduced that most of the days had 24 observations where as some of them have more than 24 .
...
With the above function we tried to remove the outliers for Temperature. We will find out the records where Temperature is falling out of the range defined in the table

...
testdata2 <- Testdata %>% 
  ...
  mutate(Conditions = sapply(Conditions,function(x) Mode(x)))
                                    #Wind_Direction = sapply(Mode(Wind_Direction)),
                                    #Conditions = sapply(Conditions,function(x) Mode(x))) 

#sapply(Date, function(x) checkWeekday(x))) 

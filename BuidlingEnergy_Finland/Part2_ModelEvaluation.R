...
### Evaluating Random forest Residual Analysis on 78 different datasets
source("Part2_RandomForest_ResidualAnalysis.R")
randomForestRes <- as.data.frame(NULL)
m = 0
for(i in Buildinglist){
  m = m + 1
  build <- unique(i["vac"])
  ...
  if(exists(paste(build,".",value,sep = ""))){
    ...
    print(paste(build,".",value,sep = ""))
    colnames(name)[which(names(name) == "consumption Kwh/sqm")] <- "consumption.Kwh.sqm"
    returnRandomForestOutlier(csv = name,ntree = 100)
